    ...
    try:
        ...
        if tool_calls:
            ...
            if tool_call["function"]["name"] == "search_engine":
                # Access the arguments - may be a string or dict
                ...
                if isinstance(arguments, str):
                    try:
                        ...
                        print(f"Error parsing arguments JSON: {arguments}")
                        return jsonify({"error": "Invalid tool call arguments"}), 500
                
                # Handle both 'query' (singular) and 'queries' (plural) for compatibility
                if "queries" in arguments:
                    search_queries = arguments.get("queries", [])
                elif "query" in arguments:
                    # If LLM returns single query, convert to array
                    ...
                    print(f"Warning: No 'queries' or 'query' found in arguments: {arguments}")
                    search_queries = []
                
                # Flatten nested structure if queries is an array of objects with nested 'queries' arrays
                flattened_queries = []
                for item in search_queries:
                    if isinstance(item, dict):
                        # Check if it has a nested 'queries' key
                        if "queries" in item and isinstance(item["queries"], list):
                            # Extract queries from nested structure
                            flattened_queries.extend(item["queries"])
                        # Check if it has a 'query' key (singular)
                        elif "query" in item:
                            flattened_queries.append(item["query"])
                    elif isinstance(item, str):
                        # Already a string, use as-is
                        flattened_queries.append(item)
                
                # Use flattened queries if we found any, otherwise use original
                if flattened_queries:
                    search_queries = flattened_queries
                
                # Ensure we have at least one query
                if not search_queries:
                    return jsonify({"error": "No search queries generated by LLM"}), 500
